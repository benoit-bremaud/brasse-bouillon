...
- Branch `feat/website-pages-deploy`, 4 commits (`09a5138`, `3374aae`, `59928ba`, `d2c35c7`), +135/-26 net. Adds the GitHub Pages deployment pipeline the monorepo had never had since the 2026-03-24 subtree merge of the archived `brasse-bouillon-website` repo. The new `.github/workflows/website-deploy.yml` triggers on push to `main` whose diff touches `packages/website/**` (and `workflow_dispatch`), stages only the public surface into `_site/` (HTML, CSS, JS, brand assets, favicon, CNAME, sitemap, robots, `seo/`, `screenshots/`, and `docs/ROADMAP.md` which is linked from the homepage), and publishes via `actions/upload-pages-artifact@v3` + `actions/deploy-pages@v4` under the canonical `github-pages` environment with `concurrency: pages`. Permissions scoped per-job (SonarLint S8264 / S8233). The companion commit `59928ba` scrubs three doc files that still claimed the obsolete standalone-website setup (`packages/website/CLAUDE.md`, `README.md`, `CONTRIBUTING.md` — references to a non-existent `website-ci-cd.yml`, a `develop` integration branch that no longer exists on the monorepo trunk, etc.).
- Round-1 review fix in `d2c35c7`: 2 inline comments addressed (1 Copilot Must Have + 1 Codex P2 Must Have). (1) Both index pages link `<a href="docs/ROADMAP.md">` but the staging script excluded the entire `docs/` directory — ROADMAP.md is now explicitly whitelisted into `_site/docs/` while the rest of `docs/` stays internal. (2) `set -euo pipefail` + bare `cp "$src"/*.png _site/` would have killed any future deploy if an asset class became empty (Bash leaves unmatched globs literal, `cp` errors with "cannot stat"); wrapped the staging block in `shopt -s nullglob` and rewrote wildcard copies as a `for f in <globs>; do cp ... ; done` loop. Required scalar files keep their direct copies; optional dirs use `if [ -d ]` guards.
- Post-merge migration executed live on 2026-05-19, captured for future replay in the new `.claude/skills/website-pages-deploy/SKILL.md`: (1) unarchive `brasse-bouillon-website` → `gh api -X DELETE …/brasse-bouillon-website/pages` (releases the `brasse-bouillon.com` CNAME from the legacy "Deploy from branch" Pages config) → re-archive; (2) `gh api -X POST …/brasse-bouillon/pages` with `{"build_type": "workflow"}` to switch Pages on for the monorepo; (3) `gh workflow run website-deploy.yml --ref main` to fire the first deploy (which provisions the HTTPS certificate); (4) `gh api -X PUT …/brasse-bouillon/pages` with `{"cname": "brasse-bouillon.com"}` to bind the custom domain once the cert exists; (5) re-trigger `workflow_dispatch` so the next deploy publishes under the new domain attachment. Final state: `https://brasse-bouillon.com/` serves `last-modified: Tue, 19 May 2026 22:11:25 GMT` with the PR #1018 "Comment ça se passe" section and the six feature-card screenshot wrappers live in HTML.
