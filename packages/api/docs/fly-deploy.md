...
| `TYPEORM_MIGRATIONS_RUN` | `true` | Migrations auto-run on boot — zero manual step |
| `TYPEORM_SYNCHRONIZE` | `false` | Never sync in production |
| `SWAGGER_ENABLED` | `false` | Production hides schema; flip to `true` for debugging |
| `SEED_ENDPOINTS_ENABLED` | `false` | No seeding in production |
