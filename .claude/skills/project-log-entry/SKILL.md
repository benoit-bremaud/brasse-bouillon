...
- **No AI *authorship* attribution** — do not credit the *change* to a tool ("Generated by Claude", "Co-authored by Codex"). This is distinct from the `Reviews —` line, which records the review *verdict* by tool name (`Claude pr-pre-reviewer + Codex CLI`, `Codex approved`, `Copilot 1 inline …`). That is an established convention of this log (used on ~90 entries) and a process fact, not authorship — keep it. Aligned with the 2026-07-02 policy allowing AI attribution for transparency.
- **No emojis** decorating headings or bullets — this includes the 👍/👎 verdict reaction: write the verdict in words (`approved`, `clean verdict`, `no findings`), never the glyph.
...
```bash
# Every SHA a heading claims is on main must be reachable from main.
# Three things make this scan correct rather than a silent no-op:
...
#   3. Test reachability, not object existence. `git cat-file -e` only proves
#      the object is in the local database — a superseded or feature-branch
#      commit still sitting there passes, so it MISSES the force-push/wrong-SHA
#      case this scan exists to catch. `merge-base --is-ancestor` is the real
#      reachability test; it also exits non-zero when the object is absent, so
#      the one check covers both "not on main" and "gone entirely".
...
grep -E '^### (PR #|Tags? |Admin bypass)' PROJECT_LOG.md \
  | grep -oE '`[0-9a-f]{7}`' | tr -d '`' | sort -u | while read -r sha; do
    git merge-base --is-ancestor "$sha" origin/main 2>/dev/null || echo "STALE: $sha"
done | head -20
```

A clean run prints nothing; any `STALE:` line points at a heading whose SHA is not reachable from `origin/main` — a real problem (force-push or branch rewrite), since heading SHAs are merge/tag/admin commits that must be present. When verifying just the entry you added, scope to the diff instead: `git diff origin/main -- PROJECT_LOG.md | grep '^+### '` then extract the same way.
