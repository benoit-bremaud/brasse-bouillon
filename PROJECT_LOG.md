...
### PR #1456 merged (`8b80d0d`) — docs(conception): retire the shop's duplicate catalog use case

- Branch `docs/shop-catalog-conception`, 3 commits (`931d15a`, `0f60a1f`, plus a context-sentence fix). Conception only, no code. Follow-up to the audit behind #1444: the app shipped two ingredient catalogs — a real one under `/ingredients` and a fake one under `/shop` — because the conception modelled the same actor goal twice. `ingredients/01-use-case.md` declared UC1–UC3 (browse / filter / datasheet) while `equipment-shop/01-use-case.md` declared UC3 "Browse the shop by category" + UC4 "View a product". Deleting the fake catalog in #1444 fixed the symptom and left the cause standing.
- `equipment-shop/01-use-case.md` retires UC3/UC4 (numbering kept stable so issue references keep resolving) and fixes the false "shipped (read)" status. `04-class.md` purges `Product` / `PriceUnit` / `ShopCategory` (deleted in #1444) and adds `ShopRayon`; `LocalCartItem.category` becomes `IngredientCategory`. `02-sequence-shopping-list.md` is marked design-not-description — its `cart.use-cases` participant no longer exists. `ingredients/01-use-case.md` records that this domain owns catalog browsing.
- New `equipment-shop/03-component.md`: the shop → catalog wiring, the endpoint map, and the two-speed rayon model.
- **Decisions**:
  - `catalog-uc-owned-by-ingredients` — one actor goal, one use case (UML 2.5): consulting the catalog is a single goal, and whether the door reads "Ingrédients" or "Boutique" is information architecture, not a goal. The ingredients domain keeps the UC; the Shop becomes its entry point. Rejected: a dedicated `shop-catalog/` conception folder, which would re-split two existing conceptions and risk recreating the overlap elsewhere.
  - `shop-hub-reuses-catalog-routes` — reuse, not relocation. `RecipeDetailsScreen` deep-links into `/(app)/ingredients/[category]/[id]` and `/(app)/ingredients/malts/[id]`, so only the `/ingredients` **hub** is retired (redirect to `/shop`); category and datasheet screens stay put. The goal is one door, not one directory.
  - `rayon-placeholder-is-an-absent-link` — `ShopRayon.catalogCategory` is nullable and that nullability *is* the two-speed hub, modelled as an absent association rather than a status flag so an unsourced rayon cannot be made pressable by accident.
- **Correction of record**: the backend already serves ten catalogs, including `/catalog/equipment-templates` and `/catalog/misc-templates`, both unconsumed mobile-side. Matériel and Accessoires are therefore waiting on mobile plumbing, not on data — only Kits has no source at all. An earlier reading of this session wrongly concluded no equipment catalog existed, having looked only at `/equipment-profiles` (the brewer's own gear — a different concept sharing the word). `/catalog/distributors` also exists, unscoped, and may be closer to a real shop than the affiliate deep-link modelled in UC5 (#650).
- Reviews — Copilot 4 inline, all fixed + replied + resolved: three flagged the PR committing the very defect it fixes (the `/ingredients` redirect, the dashboard entry removal and the hub routing described in the present tense though unbuilt), one flagged `ShopRayon` notation drift (`~nullable~` and `+method() ReturnType` per `recipe-difficulty/04-class.md`). A self-review sweep afterwards caught two further present-tense claims Copilot had missed. CI green. Follow-up: Lot 1 (shop hub becomes the catalog door for the three wired rayons), then Lot 2 (wire misc-templates, then equipment-templates).

### PR #1453 merged (`58fdbb4`) — test(shop): guard the deferred-commerce invariant; drop vestigial row

- Branch `chore/shop-guard-invariant`, 3 commits (`32f2aed`, `a3adc66`, plus a format fix). Follow-up to a post-merge audit of the shop feature (`architecture-guardian` + `test-auditor` as independent lenses). Both cleared the #1444 refactor — 0 Must Have, nothing over-engineered, no orphaned code, no value lost (verified by grep: the 11 deleted symbols have zero occurrences repo-wide) — and converged on one real gap.
- **The gap**: nothing guarded the decision the shop exists to express. A tile could regain an `onPress` and all four tests stayed green. The one absence-test present asserted `queryByLabelText("Accéder à l'Académie")` — a string that existed nowhere but in the test asserting its absence (a tombstone), and inversely correlated with the bug it claimed to guard: the Academy pill's defect was layout clipping (#1449), invisible to RTL, so re-adding the pill *correctly* in `ListHeader`'s action slot would have reddened it for a non-bug.
- Two invariant guards replace it — no pressable affordance while commerce is deferred, and no price rendered — and both were **mutation-tested** rather than assumed: a tile given `onPress` reds the first and only the first; a reintroduced `"à partir de 4,50 €"` reds the second and only the second. Three overlapping render tests collapse into one (4 tests → 3).
- `IngredientsTab`'s list row was a `flexDirection: row / space-between` container built to sit the ingredient text beside the add-to-cart button deleted in #1444; with one child left, the row properties and `listItemMain`'s `flex: 1` were no-ops. The `Pressable` is now the row — the touch target grew to the full row width.
- Also fixes a French/English accessibility split in the same element: the ingredient row announced "Open ingredient details for X" between two French labels, switching a screen reader's language mid-screen. Now "Ouvrir la fiche de X"; the two tests pinning the English string are updated with it. The same defect remains elsewhere ("Go back to scanner", "Show more pending captures") — out of scope, worth a dedicated a11y-language sweep.
- Reviews — local pre-push: 0 Must Have. Copilot 1 inline (the a11y language split), fixed + replied + resolved. CI green; 212/212 across 24 suites. Visual QA on `bb_pixel_academy` (emulator-5556) with Metro isolated on port 8082 so a concurrent session holding 8081 was left untouched — `setprop metro.host localhost` was required to defeat the `10.0.2.2:8081` default that would otherwise have served the other session's bundle. Ingredient list pixel-identical; tapping a row still opens its datasheet.
- **Note for Lot 1**: the "no pressable affordance" guard must be **amended, not deleted** when live rayons become pressable by design — it narrows to the placeholders. That re-decision is the guard working as intended.
...
- Flush edge-to-edge bar absorbing the safe-area inset, hiding on sustained scroll-down and revealing on scroll-up (instantly under reduced motion). New `useScrollDirection` (threshold gating both directions, near-top guard, end-of-list forced reveal) feeds a single `FooterVisibilityContext` driving bar + Snackbar + sticky CTA.
- `NAV_BAR_HEIGHT` (`theme/layout`) + `useNavigationBarFootprint()` are the single source: the bar's height, its hide-translate distance and the reserved clearance all derive from it (closes audit findings M2/M3, including the hardcoded `+ 96`). New `ScreenScrollView` / `ScreenFlatList` own the clearance and wire the scroll handler in one swap; the 36 per-screen paddings and `useNavigationFooterOffset` are deleted. The clearance merges last, so a screen can no longer cancel it.
- ADR-0029 status Proposed → Accepted, added to the root `CLAUDE.md`.
- **Decisions**:
  - `footer-clearance-on-scroll-containers` — amends ADR-0029 clause 4, which was not implementable as written: `Screen` wraps a screen and can only pad its own container, which stops content scrolling under the bar and turns D2's residual gap into a permanent one — the rejected option A. The clearance belongs on the scrolled content, so shared scroll containers own it; `Screen` keeps only the top clearance. Recorded in-place in the ADR with its reasoning; the three diagrams follow.
  - `sticky-cta-absolute` — the sticky CTA had to leave the layout flow to honour clause 5: static it left a dead block under the button once the bar hid, translated it dragged its box down and opened a gap above it (both observed on device). Pinned to the bottom edge like the Snackbar, it translates cleanly; its three screens reserve its height via the new `extraBottomClearance` prop, passed explicitly rather than read from the app-level sticky-CTA context (which would inflate any other mounted tab screen's padding).
  - `followers-reclaim-visual-height-only` — the bar translates by the full footprint (it leaves the screen), but its followers translate by `NAV_BAR_HEIGHT` only: the safe-area inset does not free up with the bar, and reclaiming it dropped controls into the home-indicator area.
- Deliberate losses of hand-tuned bottom room, both being the per-screen tuning this ADR centralizes: `AcademyTopicDetailsScreen` drops a `+ spacing.xl`, the CTA screens drop the `+ 96`. Neither can be re-added per screen (the containers merge the clearance last); if wanted, they belong in the shared containers.
- Reviews — local pre-push (Claude + Codex): 2 Must Have (sticky CTA not following visibility; `ScreenFlatList` untested) both fixed, the new test then catching an API drift against its twin. Copilot (requested deliberately, 57-file diff): 1 Must Have — followers translating by the full footprint cancelled the safe-area inset, missed by both other reviewers and by a visual pass — fixed in `11e4d9f`; plus 7 style-memoization comments declined with rationale (contradicts the project's profile-first memo rule; the style array is re-created regardless; no memoized consumer). CI green; live QA on the Android emulator in demo mode: bar flush, hides mid-list at full height, reveals at list end, last row clears the bar where the pill previously clipped it.
- Known follow-ups: `LabelSelectBatchScreen` keeps a raw `FlatList` (CTA outside the list, bar stays pinned there — ADR fallback B locally); `RecipeStickyCta` deserves promotion to `core/ui`; M4 (dual nav mount) stays out of scope per clause 9.
