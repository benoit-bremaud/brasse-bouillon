...
> repo ADR-0005 (backend split), repo ADR-0013 (canonical model + conception order)

## Context

**Vue cible (vision)** du périmètre fonctionnel du backend encyclopédie : tous les buts
qu'on **veut** offrir, pas seulement ceux déjà livrés. Ce diagramme est **contractuel** —
le code doit s'y conformer ; toute évolution se fait **des deux côtés** (conception *et*
réalisation), jamais l'un sans l'autre. Les cas non encore implémentés sont **marqués**
(pointillés / `<<planned>>`) pour permettre le contrôle de conformité conception ↔ code.
...
service** (domaine Recette). Les deux méthodes de scan (UC4 code-barres, UC5 étiquette)
ne servent qu'à **identifier** une bière.

## Diagram (Mermaid — aperçu rapide)
...
- **Planifié** (`<<planned>>`) : UC5 (identification par étiquette — vision, étude `scan/`),
  UC6 (proposer une correction), UC9 (modérer). Suivi : issue #1149.
...
- **Sécurité** : les écritures (UC7, UC8, UC6/UC9) supposent un acteur authentifié/autorisé.
  Le code Python n'impose aucune auth aujourd'hui (divergence + faille) — voir #1151 ; la
  maintenance passera par une interface web admin dédiée, jamais par le mobile.
...
- **Relations :** **«extend» UC4**. La validation des suggestions de scan est une modération distincte de UC9 (à réconcilier via l'étude `scan/` + interface admin).
...
- **Précondition :** Mainteneur authentifié et autorisé
...
- **Acteur principal :** Mainteneur
- **Précondition :** Mainteneur autorisé ; jeux de référence d'autorité disponibles
...
- **Acteur principal :** Mainteneur
- **Précondition :** Mainteneur authentifié ; des corrections « pending » existent (issues de UC6)
- **Garantie minimale :** aucune donnée publiée modifiée sans décision explicite ; traçabilité (qui / quand)
- **Garantie de succès :** chaque correction traitée passe à « approved » (valeur appliquée) ou « rejected »
- **Scénario nominal**
  1. Le Mainteneur ouvre la file des corrections « pending ».
  2. Il examine une correction (entité, champ, ancienne / nouvelle valeur, raison).
  3a. Il **approuve** → le système applique la nouvelle valeur et passe la correction à « approved » (reviewer + date).
  3b. Il **rejette** → statut « rejected » (reviewer + date), entité inchangée.
- **Extensions**
  - 2a. Correction obsolète (la donnée a déjà changé) → rejet motivé.
- **Postcondition :** correction « approved » ou « rejected », horodatée ; en cas d'approbation, la donnée publiée reflète la correction.
- **Relations :** association Mainteneur. Lien UC6 → UC9 = cycle de vie (`05-state`). La validation des **suggestions de scan** (UC5/3a) est une modération **distincte** — à réconcilier via l'étude `scan/` + interface admin (#1151 / interface admin).
