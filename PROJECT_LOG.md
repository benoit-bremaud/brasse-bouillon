...
- Branch `claude/kind-napier-bd7802`, 1 commit. The pre-push Codex helper (`scripts/codex-review.sh`) failed with `error: unexpected argument '--output-last-message' found`. Root cause = version-skew, not credits: the older codex-cli (0.87.x) parses `--output-last-message` only at the `exec` level and rejects it after the `review` subcommand. Fix passes the flag before `review` (`codex exec --output-last-message "$OUT" review …`), the canonical spot for a whole-run output flag; verified on the installed 0.144.1, which accepts it in both positions, so the placement is cross-version-safe. `--base`+custom PROMPT stay mutually exclusive (clap-confirmed) → the diff scope keeps riding in the prompt, so the `--base`/`--out` contract used by the `pre-push-review` skill is unchanged. `--help` now prints the full header via `awk` (drift-proof, was `sed -n '2,25p'`). Verified: `bash -n` + `shellcheck` clean, live `codex exec review` run writes the report end-to-end. `~/.codex/config.toml` `review_model`/`model` = `gpt-5.5` confirmed already correct (both valid on 0.144.1).
- Reviews — pre-push ritual (`pr-pre-reviewer` + Codex helper reconciled): 0 Must Have; 2 Should Have fixed before push (header note reworded to stop falsely claiming 0.144.1 rejects review-level `-o`; commit scope `codex-review`→`review` to match prior precedent for this file). Copilot: overview only, 0 findings; GitHub-Codex silent (flaky). CI green (`mergeStateStatus: CLEAN`), 0 inline threads.
