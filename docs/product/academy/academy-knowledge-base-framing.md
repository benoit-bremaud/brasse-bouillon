...
- Public website publishing in the original mobile V1 delivery scope.

The V1 must be shaped so these can be added later without replacing the content
model.

### Public Website Extension (2026-07-27)

Issue #993 authorizes a public, French-first guide hub as a later delivery slice.
This does not change the canonical source or introduce a CMS. The website reads
a deterministic JSON representation generated by the same parser and validator
as the mobile application.

Publication is opt-in per article through `web_publication` front matter. A
public article must also be published in the Academy corpus and carry validated
review confidence. Its stable URL is `/guides/<web-publication-slug>/`. Content
without this metadata remains unavailable to the public website.

The initial website slice must not generate English equivalents until complete,
editorially useful translations exist.
