...
Shows the **target** structure that replaces today's floating-pill + per-screen offset. It makes explicit the single new seam — a shared scroll-direction contract feeding one footer-visibility source — and the fact that the bottom clearance moves **out of the 41 screens** into shared scrollable primitives. It does NOT show the runtime timing (see `03-sequence`) nor the visibility lifecycle (see `02-state`).

Today's coupling this removes: 36 opt-in screens (plus the app-level Snackbar) each call `useNavigationFooterOffset()` and wire their own `paddingBottom`; 5 more never opted in at all and are silently occluded — 41 in total (audit findings M1/M2); much of the `sticky-cta-clearance` provider exists to work around the floating pill's offset arithmetic.
...
flowchart TD
  ...
  subgraph core ["src/core/ui + core/theme"]
    Screen["ScreenScrollView / ScreenFlatList (own the constant bottom footprint)"]
  ...
  subgraph features ["src/features/**/presentation (41 scrollable screens)"]
...
- **The shared scrollable primitives own the bottom clearance** (`ScreenScrollView` / `ScreenFlatList`) — `Screen` keeps only the top clearance (`brandHeader.contentClearance`), because it does not own its children's scroll container (ADR-0029 clause 4, amended). This removes the per-screen opt-in that is the root cause of "it hides buttons" (**M1/B1**) and covers the 5 screens that had never opted in. `useNavigationFooterOffset` is deleted.
